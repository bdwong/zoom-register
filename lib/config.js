...
function copyConfig() {
   ...
   const src = config_template_path();
   const dst = config_filepath();

   console.log(`Copying template config to ${dst}`); //from ${src}
   const data = readFileSync(src);
   writeFileSync(dst, data);
   // copyFileSync(src, dst); // It's an error to copy from packge to disk directly.
